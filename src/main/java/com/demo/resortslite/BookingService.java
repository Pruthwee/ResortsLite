...
public class BookingService {
    ...
    // Externalized payment API endpoint via environment variable, using HTTPS
    private static final String PAYMENT_API = System.getenv().getOrDefault("PAYMENT_API_ENDPOINT",
            "https://payment-svc.internal:8443/payments/charge");

    public Map<String, Object> createBooking(String guestName, String roomType,
                                              String checkIn, String checkOut) {
        String bookingId = "BK-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();

        // Fixed: SQL injection — using parameterized query with placeholders
        String sql = "INSERT INTO bookings (id, guest, room, checkin, checkout) VALUES (?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, bookingId, guestName, roomType, checkIn, checkOut);

        // Fixed: Replaced MD5 (broken hash) with SHA-256
     ...
     * @param season   the season (PEAK, OFF, or other)
     * @param loyalty  the loyalty tier (GOLD, PLATINUM, DIAMOND, or other)
    ...
    public String calculateRoomPrice(String roomType, int nights, String season, String loyalty) {
        ...
        basePrice = applyLongStayDiscount(basePrice, nights);
    ...
    private double applyLongStayDiscount(double basePrice, int nights) {
        if (nights >= 14) {
    ...
    private boolean isValidRoomType(String roomType) {
        return roomType != null &&
                (roomType.equals("STANDARD") || roomType.equals("DELUXE")
